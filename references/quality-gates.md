...
### Unified QQ Channel / QQ Group / QQ Campus Wall

| Check | BLOCKER | WARNING | Action |
|---|---|---|---|
| CTA unclear | Reader does not know whether to报名、扫码、进群、到场、评论. | CTA appears but is buried. | Put action near top and bottom. |
| Too polished for QQ | Copy feels like a WeChat article pasted into QQ. | Some long paragraphs remain. | Use shorter blocks, clear bullets, and direct invitation. |
| Missing interaction prompt | No question or invitation to reply/咨询. | Prompt exists but vague. | Add "想路演/想组队/想旁听都可以来". |
| Campus wall risk | Copy includes sensitive or unconfirmed internal details unsuitable for public wall submission. | Copy is public-safe but too long. | Use public-safe, concise version. |
