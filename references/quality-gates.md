...
| Article coverage passed | v2 HTML is generated from a fact table/summary, lacks `Article-to-HTML Mapping`, omits major article sections, or covers less than about 80% of substantive article units. | Regenerate HTML from the complete WeChat article. |
...
| Missing article mapping | `Article-to-HTML Mapping` is absent, or maps from the fact table instead of the complete WeChat article. | Mapping exists but is too coarse to prove article coverage. | Break the WeChat article into units and map each unit to an HTML component. |
| Article coverage failed | HTML omits major article units such as hook, background, participant details, rules, agenda explanations, or closing invitation. | Coverage is high but a minor repeated paragraph is merged. | Add omitted units back into HTML or explain safe duplication removal. |
